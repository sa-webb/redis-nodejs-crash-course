...
router.get(
  '/user/:userId',
  [
    param('userId').isInt({ min: 1 }), // make sure its an int
    apiErrorReporter,
  ],
  async (req, res) => {
    const { userId } = req.params; // pull out params
    const userKey = redis.getKeyName('users', userId);

    const userDetail = await redisClient.hgetall(userKey); // return all values of key as object
